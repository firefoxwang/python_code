...
def delete():
    ...
    try:
        ...
        cur.close()
        conn.close()
        print 'delete sql server ok,and begin wallet delete '
        cur2.execute("DELETE FROM wallet_uat.apply_info WHERE cellphone = %r" % mobile)
        cur2.execute("DELETE FROM wallet_uat.apply_info WHERE cellphone =  %r" % mobile)
        cur2.execute("DELETE FROM wallet_uat.money_box WHERE member_id = %d" % memberid_int)
        print "--------------"
        print 'mysql delete ok'
        conn2.rollback()
        cur2.close()
        conn2.close()
